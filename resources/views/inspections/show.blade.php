...
<style>
    /* CSS Override untuk tukar warna Pagination ke tema Emerald */
...
<div class="max-w-4xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Butiran Projek Pemeriksaan</h2>
            <p class="text-gray-500 text-sm mt-1">Maklumat lengkap hartanah dan klien.</p>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('inspection.edit', $inspection->id) }}" class="px-4 py-2 bg-amber-500 text-white text-sm font-semibold rounded-lg hover:bg-amber-600 shadow-sm">
                <i class="fa-solid fa-pen-to-square mr-1"></i> Edit Projek
            </a>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <span class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Tajuk Projek</span>
                <p class="text-lg font-bold text-gray-900 mt-1">{{ $inspection->title }}</p>
            </div>
            <div>
                <span class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Jenis Hartanah</span>
                <p class="mt-1"><span class="px-3 py-1 bg-blue-50 text-blue-700 text-xs font-medium rounded-full">{{ $inspection->type }}</span></p>
            </div>
            <div>
                <span class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Nama Klien</span>
                <p class="text-base font-semibold text-gray-800 mt-1">{{ $inspection->clientname }}</p>
            </div>
            <div>
                <span class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Negeri</span>
                <p class="text-base text-gray-800 mt-1 font-medium">{{ $inspection->state }}</p>
            </div>
            <div class="col-span-1 md:col-span-2">
                <span class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Alamat Penuh</span>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-gray-100">
            <div>
                <span class="text-xs font-semibold text-gray-400 uppercase tracking-wider block mb-2">Gambar Rumah (Muka Depan)</span>
                @if($inspection->img)
                    <img src="{{ asset('storage/' . $inspection->img) }}" alt="Gambar Rumah" class="w-full h-64 object-cover rounded-xl shadow-sm border">
                @else
                    <p class="text-sm text-gray-400 italic">Tiada gambar dimuat naik.</p>
                @endif
            </div>
            <div>
                <span class="text-xs font-semibold text-gray-400 uppercase tracking-wider block mb-2">Pelan Layout</span>
                @if($inspection->layout_img)
                    <img src="{{ asset('storage/' . $inspection->layout_img) }}" alt="Pelan Layout" class="w-full h-64 object-cover rounded-xl shadow-sm border">
                @else
                    <p class="text-sm text-gray-400 italic">Tiada pelan layout dimuat naik.</p>
                @endif
            </div>
        </div>
    </div>
...
<div class="max-w-4xl mx-auto mt-6">
    <div class="flex flex-col sm:flex-row justify-end gap-3">
        <a href="{{ route('inspection.pdf', ['id' => $inspection->id, 'template_type' => 'template1']) }}" class="px-5 py-2.5 bg-red-50 text-red-600 hover:bg-red-100 font-bold rounded-xl text-sm transition-colors shadow-sm border border-red-200 flex items-center justify-center">
            <i class="fa-solid fa-file-pdf mr-2 text-lg"></i> Generate PDF (Klasik)
        </a>
        <a href="{{ route('inspection.pdf', ['id' => $inspection->id, 'template_type' => 'template2']) }}" class="px-5 py-2.5 bg-blue-50 text-blue-600 hover:bg-blue-100 font-bold rounded-xl text-sm transition-colors shadow-sm border border-blue-200 flex items-center justify-center">
            <i class="fa-solid fa-file-pdf mr-2 text-lg"></i> Generate PDF (Jadual)
        </a>
    </div>
</div>
...
<div class="max-w-6xl mx-auto mt-8">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
            <div>
                <h3 class="text-lg font-bold text-gray-800">Senarai Defect & Kerosakan</h3>
                <p class="text-sm text-gray-500">Jumlah defect direkodkan: <span class="font-bold text-blue-600">{{ $inspection->defects->count() }}</span></p>
            </div>
            <a href="{{ route('defects.rapid', $inspection->id) }}" class="px-4 py-2 bg-emerald-100 text-emerald-700 hover:bg-emerald-200 font-bold rounded-lg text-sm transition-colors whitespace-nowrap">
                <i class="fas fa-plus mr-1"></i> Tambah Defect
            </a>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider">
                        ...
                        <th class="py-3 px-6 font-semibold whitespace-nowrap">Lokasi</th>
                        <th class="py-3 px-6 font-semibold min-w-[120px]">Kategori / Jenis</th>
                        <th class="py-3 px-6 font-semibold min-w-[150px]">Isu (Defect)</th>
                        <th class="py-3 px-6 font-semibold whitespace-nowrap">Gambar</th>
                        <th class="py-3 px-6 font-semibold text-center whitespace-nowrap">Tindakan</th>
                    </tr>
                </thead>
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="py-4 px-6">
                                <div class="flex gap-1 flex-wrap min-w-max">
                                    @if($defect->img && is_array($defect->img))
                                        ...
                                        @endforeach
                                    @else
                                        <span class="text-xs text-gray-400">Tiada</span>
                                    @endif
                                </div>
                            </td>
                            
                            <!-- BAHAGIAN TINDAKAN DIBETULKAN (flex-col untuk mobile) -->
                            <td class="py-4 px-6 text-center">
                               <div class="flex flex-col sm:flex-row justify-center items-center gap-2 min-w-max">
                                    ...
                                    <!-- Butang Delete -->
                                    <form action="{{ route('defects.destroy', $defect->id) }}" method="POST" onsubmit="return confirm('Anda pasti mahu memadam defect ini?');" class="w-full sm:w-auto block">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="py-8 text-center text-gray-400">
                                ...
                                Belum ada sebarang defect direkodkan untuk projek ini.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
